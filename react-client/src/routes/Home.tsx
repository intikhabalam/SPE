            ...
            <ol className="setup-steps">
                ...
                {isSignedIn && (
                    <li>
                        ...
                        <span> this app's Container Type on your tenant. You may need to ensure to enable SPE in the SharePoint Admin center. Sometimes this registration will fail on the first try. If that happens, sign out of this app, sign in again, redo admin consent, and retry this registration.  </span>
                        {registering && (<p>Registering...</p>)}
                        {!registering && (<p><code>{registerResult}</code></p>)}
                    </li>
                )}
                {isSignedIn && (<li>Visit the <Link to="/jobs">Jobs</Link> page to use the demo app</li>)}
            </ol>
        </div>
    );
}
