...
import { Folder24Filled, Checkmark16Filled } from "@fluentui/react-icons";
...
export async function loader({
  params,
}: ILoaderParams): Promise<IContainer | undefined> {
...
export const ContainerBrowser: React.FunctionComponent = () => {
  ...
  const [folderPath, setFolderPath] = useState<IDriveItem[]>(
    [] as IDriveItem[]
  );
...
*/
  useEffect(() => {
    (async () => {
      ...
      filesApi
        .listItems(container.id, parentId)
        .then(setDriveItems)
        .catch(console.error);
    })();
  ...
  const onBreadcrumbClick = (folder: IDriveItem) => {
    while (
      folderPath.length > 0 &&
      folderPath[folderPath.length - 1].id !== folder.id
    ) {
      folderPath.pop();
    }
  ...
  const getItemName = (driveItem: IDriveItem): JSX.Element => {
    ...
    if (driveItem.folder) {
      return (
        <Link onClick={() => onFolderClicked(driveItem)}>{driveItem.name}</Link>
      );
    }
    return (
      <Link onClick={() => onFilePreviewSelected(driveItem)}>
        {driveItem.name}
      </Link>
    );
  };
        ...
        <>
          ...
          <div className="container-breadcrumb">
            <Breadcrumb size="large">
              <BreadcrumbItem>
                <BreadcrumbButton size="large" onClick={() => setLocation([])}>
              ...
              {folderPath.map((folder) => (
                <React.Fragment key={folder.id}>
                  <BreadcrumbDivider />
                  <BreadcrumbItem>
                    <BreadcrumbButton onClick={() => onBreadcrumbClick(folder)}>
                      {folder.name}
                    </BreadcrumbButton>
                  </BreadcrumbItem>
                </React.Fragment>
