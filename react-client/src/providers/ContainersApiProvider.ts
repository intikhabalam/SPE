...
export class ContainersApiProvider {
  ...
  public static readonly instance: ContainersApiProvider =
    new ContainersApiProvider();
  ...
  public async create(
    container: IContainerClientCreateRequest
  ): Promise<IContainer> {
    const request: RequestInit = {
      method: "POST",
      headers: this._headers(await this.authProvider.getToken()),
  ...
  public async update(container: IContainer): Promise<IContainer> {
    ...
    const containerUpdate: IContainerUpdateRequest =
      container as IContainerUpdateRequest;
